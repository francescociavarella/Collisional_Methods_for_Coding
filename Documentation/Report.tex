...
\begin{document}
	...
	\begin{itemize}
		\item \texttt{pop\_traj\_sample} : 3D array with dimension $ [N_{Site}, N_{t\, Steps}, N_{SampleTraj}] $; stores the population of each site at every time step for 3 independent sampled trajectory.
		\item \texttt{avg\_pop\_traj} : 2D array with dimension $ [N_{Site}, N_{t\, Steps}] $; stores the average value over all different trajectories of each site's Population at every time step.
		\item \texttt{sample\_x, sample\_y, sample\_z} : 2D arrays with dimension $ [N_{t\, Steps}, N_{SampleTraj}] $; store from 3 sampled trajectories the Components of the \textit{Bloch State Vector} projected onto the subspace of the Single-Exciton states and computed via the \texttt{compute\_Bloch\_Sphere} function, valid specifically for the Exciton Dimer case.
		\item \texttt{avg\_x, avg\_y, avg\_z} : 1D arrays with dimension $ [N_{t\, Steps}] $; stores average value over different trajectories of the \textit{Bloch State Vector} components.
	...
	Once the computational data has been generated and stored in the \texttt{results} Dictionary, the analysis is performed using two dedicated Jupyter Notebooks:
	
	\subsection{3D Visualization: \texttt{Bloch\_Sphere.ipynb}}
	This notebook provides a geometrical interpretation of the dynamics within the Single-Exciton subspace, mapping the state evolution onto the **Bloch Sphere**. The expected visualization highlights the fundamental difference between Single Realizations and Average over Trajectories:
	
	\begin{itemize}
		\item Ensemble Average (Green Vector): This Vector represents the density matrix $\rho_S(t)$. Due to the decoherence induced by the environment, its length is expected to decrease over time (shrinking inside the sphere).
		
		\item Single Trajectories (Blue Points): These represent the pure states $\ket{\psi(t)}$ of individual stochastic Trajectory. They must always lie on the surface of the sphere (norm = 1), as the evolution remains unitary.
	\end{itemize}
	
	The visual behavior of the blue points distinguishes the two dynamical limits:
	\begin{itemize}
		\item In the \textit{Quantum Jump Limit}, the state mostly follows the Coherent Unitary Rotation but occasionally undergoes sudden, discontinuous "jumps".
		\item In the \textit{Diffusive Limit}, the Trajectory is continuous but "noisy". The state performs a Brownian-like motion on the sphere's surface, fluctuating constantly around the ideal unitary path.
	\end{itemize}
	
	\subsection{2D Visualization: \texttt{Plot.ipynb}}
	The second notebook focuses on the 2D plotting of populations and convergence metrics. Based on the generated data, the analysis focuses on four main comparisons:
	
	\begin{enumerate}
		\item  \textit{"Comparison Lindblad, Trace, Avg Trajectories"} : verify the validity of the method. The Average of the Stochastic Trajectories and the Partial Trace method must perfectly overlap with the benchmark solution provided by the Lindblad Master Equation.
		
		\item \textit{"Comparison Trajectories: Collisional vs Isolated"} : visualizes the effect of $ \mathcal{H}_{Coll} $. While the Isolated System shows perfect Rabi oscillations, the Collisional Trajectory exhibit always present noise (Diffusive) or occasionally jump (Quantum Jump) .
		
		\item \textit{"Comparison different $N_{traj}$"} : demonstrates that as the trajectories number increases (e.g., from 100 to 10,000), the residual noise in the average population decreases, converging to the analytical Lindblad result.
		
		\item \textit{"Comparison different dt"}: validates the continuous limit. It show that as $\Delta t \to 0$, the Discrete Collisional Dynamics correctly reproduce the continuous differential equations of the system.
	\end{enumerate}

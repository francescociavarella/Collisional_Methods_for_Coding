...
\begin{document}
	...
	\begin{description}
		\item[Quantum Jump Algorithm]
			\begin{enumerate}
				\item Isolated System's evolution with $ \mathcal{U}_{Exc} $
				\item Extraction for every different site of a Random Number $\alpha$ in [0,1]
				\item If $ \alpha < \sin^{2}{\left(c_j \Delta t\right)} $ it means that there's been an effective collision that has flipped the initial Ancilla's state $ \ket{0_a} $ to $ \ket{1_a} $ and so the System's wf get modified by $ \sigma_{z}^{j} $ on every \textit{j-th} site; \\
				if $ \alpha > \sin^{2}{\left(c_j \Delta t\right)} $ the system remain unchanged (i.e. application of $ \mathbb{I}^j $ )
				\item Measure and store System's Observables at that time step (like population)
				\item Repeat for the next time step the algorithm
			\end{enumerate}
	\end{description}
